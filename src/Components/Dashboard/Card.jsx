...
const Card = ({ title, value, icon, change, changeIcon }) => {
    const getIcon = (icon) => {
        switch (icon) {
            case "orders":
                return (
                    <FaShoppingBasket className="text-[#455CD7] bg-[#29326A] p-2 rounded-lg" />
                );
            case "delivered":
                return (
                    <HiShoppingBag className="text-[#00CA8C] bg-[#125447] p-2 rounded-lg" />
                );
            case "cancelled":
                return (
                    <HiMiniShoppingBag className="text-[#F75C5D] bg-[#613337] p-2 rounded-lg" />
                );
            case "revenue":
                return (
                    <TbShoppingCartDollar className="text-[#E149A3] bg-[#5A2C4C] p-2 rounded-lg" />
                );
            default:
                return null;
        }
    ...
    return (
        <Container>
            ...
            <div className="flex justify-between items-center">
                <Value>{value}</Value>
                <div
                    className={`text-sm flex items-center font-semibold ${getChangeColor(
                        changeIcon
                    )}`}
                >
